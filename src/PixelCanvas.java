...
public class PixelCanvas extends JPanel {
    ...
    static int cameraXPosition = 0; // This variable is used to decide where the "camera" is in the gameworld on the horizontal plane, i.e what pixels to show to the player
    ...
    private String[][] gameWorld = new String[BACKGROUND_PIXEL_HEIGHT][BACKGROUND_PIXEL_WIDTH];
    private String[][] visibleGameWorld = new String[GAME_PIXEL_HEIGHT][GAME_PIXEL_WIDTH];
    ...
    public void setCharacterFromFile() throws IOException {
        ...
        BufferedImage image = ImageIO.read(file);
        for (int i = 0, s = 0; i < CHARACTER_PIXEL_HEIGHT; i++, s++) {
            for (int j = 0; j < CHARACTER_PIXEL_WIDTH; j++, s++) {
                int color = image.getRGB(j, i);
                int alpha = (color >> 24) & 0xff;
                int red = (color >> 16) & 0xff;
                int green = (color >> 8) & 0xff;
    ...
    public void moveCameraWithPlayer(){
        if (player.getPlayerDistanceFromLeftBorder()>120)
            moveCameraRight();

        if (player.getPlayerDistanceFromLeftBorder()<60)
            moveCameraLeft();
    }
